...
class PdoStd
{
    ...
    public function execQuery($data=[],$sql='',$isResult=false,$limit=0){
       ...
       // echo "$sql $updateValue $joinSql $whereValsalue $orWhereValsalue $orderBy $groupBy ".$this->having[$this->currentDbhName]." $limit";
      // exit;
        //$this->dbh->setAttribute(\PDO::ATTR_EMULATE_PREPARES, true);
        $stmt = $this->dbh[$this->currentDbhName]->prepare("$sql $updateValue $joinSql $whereValsalue $orWhereValsalue $orderBy $groupBy ".$this->having[$this->currentDbhName]." $limit");
     ...
     * 获得不重复的值
     */
    public function distinct($state=true,$fields='')
    {
        $this->distinctStr[$this->currentDbhName] = rtrim($fields,',');
        $this->distinct[$this->currentDbhName] = $state;
    }
    ...
    public function reset(){
        ...
        $this->limit[$this->currentDbhName] = 1;
